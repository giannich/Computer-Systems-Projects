...
# Adds the classes names to the className list and method names to the methds list, it is sorta like a pre-processor
for file in fileList:

    if file.endswith('.jack'):

        # Adds class Names
        compilation.addClass(file[:-5])

        # Adds method Names
        inputfilePath = inputDirectory + '/' + file
        inputFile = open(inputfilePath, 'r')
        fileStream = getStream(inputFile)

        compilation.addMethods(file[:-5], fileStream)
